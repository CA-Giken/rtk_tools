...
greencolor=Config["color"]["green"]
yellowcolor=Config["color"]["yellow"]
redcolor=Config["color"]["red"]
...
for key in ckeys:
  if key.startswith('indic'):
    item=Config[key]
    n=len(Indicates)
    typ=item["type"].split('/')[-1]
    exec("val="+typ+"()")
    if (not isinstance(val,Bool)):
      txt="{}:{}".format(item["label"],"---")
    else:
      txt=item["label"]
    wlabel=tk.Label(root,text=txt,font=normalfont,background=maskcolor,foreground=unlitcolor)
    wlabel.pack(side='right',fill='y',anchor='e',padx=(0,5))
    item["tag"]=wlabel
    exec("rospy.Subscriber('"+item["topic"]+"',"+typ+",functools.partial(cb_indicator,n))")
    rospy.set_param('/dashboard/ind'+item["topic"],False)
