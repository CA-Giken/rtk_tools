...
class rtkEzui(object):
  ...
  def same_on(self,pane):
    ...
    self.btn_bwd=tk.Button(self.ctrl,image=self.larricon,command=self.cb_pagebwd)
    self.btn_bwd.grid(row=0,column=0,padx=1,pady=1,sticky='nsew')
    ...
    self.btn_fwd=tk.Button(self.ctrl,image=self.rarricon,command=self.cb_pagefwd)
    self.btn_fwd.grid(row=0,column=2,padx=1,pady=1,sticky='nsew')
    tk.Label(self.ctrl,font=(self.prop["font"]["family"],self.prop["font"]["size"]),anchor='c').grid(row=0,column=3,padx=1,pady=1,sticky='nsew')
    if self.prop["dump"]!="": 
      self.btn_sav=tk.Button(self.ctrl,image=self.saveicon,command=self.cb_save)
      self.btn_sav.grid(row=0,column=4,padx=1,pady=1,sticky='nsew')
    rtkPage.show(0)
    self.btn_bwd.focus()
